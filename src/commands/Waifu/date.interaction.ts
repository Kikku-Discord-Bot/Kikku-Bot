...
export class DateSlashCommand extends BaseSlashCommand {
	...
	async execute(client: BaseClient, interaction: ChatInputCommandInteraction): Promise<void> {
		...
		const dbHandler = client.getModule("Waifu").getDatabaseHandler();
		const dbWaifu = await dbHandler.getRowUser({userId: interaction.user.id, waifu: jikanResult.name});
		if (!dbWaifu) {
			await dbHandler.createRowUser(
				{
